...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Verificar parámetro de logout para evitar bucles y limpiar estado residual
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.has('logout')) {
        console.log('Sesión cerrada exitosamente - Limpiando estado residual');
        ...
        // Opcional: Mostrar notificación de logout exitoso
        setTimeout(() => {
    ...
    if (isLoginPage) {
        // Solo en login: si ya está autenticado, redirigir al menú
        try {
            console.log("Verificando si ya está autenticado...");
            const info = await me();
            if (info?.authenticated && info.user) {
                console.log("Usuario ya autenticado, redirigiendo a menu...");
                window.location.href = "menu.html";
                return;
            }
        } catch (error) {
            console.log('No hay sesión activa, continuar con login normal');
        }
    }
    ...
    function mostrarNotificacion(mensaje, tipo = "exito") {
        ...
        if (!notificacion || !notificacionMensaje) {
    ...
    function mostrarModal(mensaje) {
        return new Promise((resolve) => {
            ...
            if (!modal || !modalMensaje || !btnSi || !btnNo) {
    ...
    if (form) {
        form.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                if (!loginResult) {
                    mostrarNotificacion("Error en login", "error");
                    return;
                }
